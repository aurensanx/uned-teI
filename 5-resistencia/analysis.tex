...
porque la resistencia de cada cable varía con la longitud $L$ y también con la sección $S$, según la expresión~\ref{eq:ressitividad}.
...
Las figuras~\ref{fig:RL1},~\ref{fig:RL2} y~\ref{fig:RL3} muestran la representación de $R$ frente a $L$
para cada cable.
...
Las figuras~\ref{fig:RS1},~\ref{fig:RS2} y~\ref{fig:RS3} muestran la representación de $R$ frente a $1/S$
...
\subsection{\label{subsec:RLS}}


La figura~\ref{fig:RLS} muestra la representación de $R$ frente a $L/S$.
...
En la sección~\ref{subsec:RL} solo se tiene en cuenta la dependencia de $R$ con $L$, en la sección~\ref{subsec:RS}
la dependencia de $R$ con $S$.

Además, con este método se tienen más medidas para poder hacer el ajuste y eso minimiza el error.

\subsection{}

Consultando la referencia~\cite{1}, observamos que el valor de la resistividad $\rho$ obtenido
en la sección~\ref{subsec:RLS} podría corresponder a 2 elementos, cobre y oro, al ser los dos únicos
elementos que aparecen dentro del rango de valores posibles.
